...
#### Methods
  - __init__ (class):
      ...
      find all transaction-template JSON files (in transfers/<foo>.json) and make
...
```
  ...
  - ask():
      ...
      interactive loop, showing all bucket names as pick list.
...
#### Methods
  - __init__(path2csv, buckets):

    read the CSV and get all its transactions
...
#### Methods
  ...
  - init_buckets(path2buckets.json):
    ...
    load up the buckets.json file and make all the buckets

  - load_sv(path2savings.csv):

    load the savings.csv file with the starting total and fill the buckets with start amounts.
